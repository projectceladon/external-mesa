...
def process_meson(file, file_name: str, output_indent: str):
    ...
    for line in content.splitlines():
        ...
        if match != None:
            ...
            print(subdir_output_indent + 'set_relative_dir(\'%s\')' % next_dir, file=file)
            process_meson(file, next_file, subdir_output_indent)
            print(subdir_output_indent + 'set_relative_dir(\'%s\')' % current_dir, file=file)
            continue
...
def generate():
    output_file_name = 'generate_android_build.py'
    print('Writing to: ' + output_file_name)

    with open(output_file_name, 'w') as file:
        print("########################################################################################################################", file=file)
        print("###", file=file)
        print("### Pull in the definitions meson is expecting to generate Android bp files", file=file)
        print("###", file=file)
        print("from meson_android import *", file=file)
        print("###", file=file)
        print("########################################################################################################################", file=file)
        print('', file=file)
        ...
        print('### Open the android build definition file', file=file)
        ...
        print('### Close the android build definition file', file=file)
