...
#include <i915_drm.h>
#include <intel/perf/gen_perf_query.h>

#include <profiler/pps.h>
#include <profiler/pps_algorithm.h>
...
bool IntelDriver::init_perfcnt()
{
   ...
   for (auto &query : perf->get_queries()) {
      ...
      for (int i = 0; i < query->n_counters; ++i) {
         gen_perf_query_counter &counter = query->counters[i];
...
uint32_t IntelDriver::gpu_next()
{
   ...
   gen_perf_query_result_accumulate_fields(&result,
      &perf->query.value(),
      &perf->devinfo,
      record_a + 1,
      record_b + 1,
