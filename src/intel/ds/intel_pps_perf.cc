...
#include <profiler/pps.h>
#include <profiler/pps_device.h>
...
IntelPerf::IntelPerf(const int drm_fd)
   ...
   , cfg {gen_perf_new(ralloc_cfg)}
{
   assert(drm_fd >= 0 && "DRM fd is not valid");

   if (!gen_get_device_info_from_fd(drm_fd, &devinfo)) {
      PPS_LOG_FATAL("Failed to get devinfo");
   }

   gen_perf_init_metrics(cfg,
      &devinfo,
      drm_fd,
...
std::optional<struct gen_perf_query_info> IntelPerf::find_query_by_name(
   const std::string &name) const
{
   for (int i = 0; i < cfg->n_queries; ++i) {
      struct gen_perf_query_info query = cfg->queries[i];
...
std::vector<struct gen_perf_query_info *> IntelPerf::get_queries() const
{
   assert(cfg && "Intel perf config should be valid");
   assert(cfg->n_queries && "Intel perf queries not initialized");

   std::vector<struct gen_perf_query_info *> queries = {};
...
bool IntelPerf::open(const uint64_t sampling_period_ns)
{
   ...
   ctx = gen_perf_new_context(ralloc_ctx);
   gen_perf_init_context(ctx, cfg, nullptr, nullptr, nullptr, &devinfo, 0, drm_fd);

   auto oa_exponent = get_oa_exponent(drm_fd, sampling_period_ns);

   return gen_perf_open(ctx,
...
bool IntelPerf::oa_stream_ready() const
{
   ...
   return gen_perf_oa_stream_ready(ctx);
}

ssize_t IntelPerf::read_oa_stream(void *buf, size_t bytes) const
{
   assert(ctx && "Perf context was not open");
   return gen_perf_read_oa_stream(ctx, buf, bytes);
}
